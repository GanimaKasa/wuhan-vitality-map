...
def is_workday(date: str, runtime: ToolRuntime):
    ...
    result = safe_call(tool_is_workday, date=date)
    return build_command("is_workday", result, runtime.tool_call_id)
...
def get_weather(date: str, runtime: ToolRuntime):
    ...
    result = safe_call(tool_get_weather, date=date)
    return build_command("get_weather", result, runtime.tool_call_id)
...
def get_vitality(runtime: ToolRuntime, period: str | None = None, district: str | None = None,
    ...
    result = safe_call(tool_get_vitality, period=period, district=district, order=order, topn=topn)
...
def search_weibo_hotspots(keyword: str, runtime: ToolRuntime, top_n: int = 20):
    ...
    result = safe_call(tool_search_weibo_hotspots, keyword=keyword, top_n=top_n)
...
def web_search(query: str, runtime: ToolRuntime):
    ...
    result = safe_call(tool_web_search, query=query)
    return build_command("web_search", result, runtime.tool_call_id)
